...
import {  useDeleteSupplierMutation, useGetAllSupplierQuery, useGetAllSupplierWithoutQueryQuery, useInsertSupplierMutation, useUpdateSupplierMutation } from "../../features/supplier/supplier";

const SuppliersTable = () => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [currentSupplier, setCurrentSupplier] = useState(null); 
    const [isModalOpen1, setIsModalOpen1] = useState(false);
    const [createSupplier, setCreateSupplier] = useState({ name: '',  remarks: '' });
    ...
    const { data, isLoading, isError, error } = useGetAllSupplierQuery({ startDate, endDate, supplierId, page: currentPage, limit: itemsPerPage });
    ...
    const handleUpdateProduct = async () => {
        ...
        try {
            const res = await updateSupplier({ id: currentSupplier.Id, data: updatedProduct }).unwrap();
            if (res.success) {
    ...
    <Select
        ...
        value={supplierOptions.find((option) => option.value === currentSupplier?.supplierId)} // Optional chaining ব্যবহার করা হলো
                ...
                <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50'>
                    <motion.div className='bg-gray-800 rounded-lg p-6 shadow-lg w-full md:w-1/3 lg:w-1/3' initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                        ...
                        <div className='mt-4'>
                            ...
                            <input type='text' value={currentSupplier?.name} onChange={(e) => setCurrentSupplier({ ...currentSupplier, name: e.target.value })} className='border border-gray-300 rounded p-2 w-full mt-1 text-black' />
                        </div>
                ...
                <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50'>
                    <motion.div className='bg-gray-800 rounded-lg p-6 shadow-lg w-full md:w-1/3 lg:w-1/3' initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                        ...
                        <form onSubmit={handlecreateSupplier}>
                            <div className='mt-4'>
                                <label className='block text-sm text-white'>Name:</label>
                                <input type='text' value={createSupplier.name} onChange={(e) => setCreateSupplier({ ...createSupplier, name: e.target.value })} className='border border-gray-300 rounded p-2 w-full mt-1 text-black' required />
                            </div>
                            {/* <div className='mt-4'>
                                <label className='block text-sm text-white'>Due Amount:</label>

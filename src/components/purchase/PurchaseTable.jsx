...
import { useDeletePurchaseMutation, useGetAllPurchaseQuery, useInsertPurchaseMutation, useUpdatePurchaseMutation } from "../../features/purchase/purchase";
...
const PurchaseTable = () => {
    ...
    const [createPurchase, setCreatePurchase] = useState({ product_name: '', supplier_name:'', transaction_date:'', quantity:0, rate:0,paid_amount:0,due_amount:0, remarks: '', supplierId:'' });
...
const { data, isLoading, isError, error } = useGetAllPurchaseQuery({ startDate, endDate, supplierId, page: currentPage, limit: itemsPerPage });
...
}, [data, isLoading, isError, error, currentPage, itemsPerPage, startDate, endDate, supplierId]);
...
console.log('purchaseData', purchaseData);
    ...
    const handleUpdateProduct = async () => {
        ...
        try {
            const res = await updateProduct({ id: currentPurchase.Id, data: updatedProduct }).unwrap();
            if (res.success) {
    ...
    const { data:data1, isLoading:isLoading1, isError:isError1, error:error1 } = useGetAllSupplierWithoutQueryQuery();
    ...
    const handleDownload = (product) => {
        ...
        const data = [
            ["Product Name", "Supplier Name", "Quantity", "Price", "Paid Amount", "Due Amount"],
            [product.product_name, product.supplier_name, product.quantity, product.price, product.paid_amount, product.due_amount]
        ];
...
console.log("productOptions", productOptions)
    ...
    <Select
        ...
        value={supplierOptions.find((option) => option.value === currentPurchase?.supplierId)} // Optional chaining ব্যবহার করা হলো
                    ...
                    <thead>
                        <tr>
                            ...
                            <th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider'>Product Name</th>
                             <th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider'>Supplier Name</th>
                    ...
                    <tbody className='divide-y divide-gray-700'>
                        {
                            ...
                            purchases.map((product) => (
                                <motion.tr key={product.Id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
                                   <td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100'>{new Date(product.transaction_date).toLocaleDateString('en-GB')}</td>
                                   <td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100'>{product.product_name}</td>
                                    <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>{product.supplier_name}</td>
                                    <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>{Number(product.quantity || 0).toFixed(3)}</td>
                                    <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>{Number(product.rate || 0).toFixed(2)}</td>
                                    <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>{Number(product.price || 0).toFixed(2)}</td>
                ...
                <div className='fixed inset-0 top-72 flex items-center justify-center bg-black bg-opacity-50'>
                    <motion.div className='bg-gray-800 rounded-lg p-6 shadow-lg w-full md:w-1/3 lg:w-1/3' initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                        ...
                        <Select
                            ...
                            value={productOptions.find((option) => option.value === currentPurchase.productId)}
                            onChange={(selectedOption) => setCurrentPurchase({ ...currentPurchase, productId: selectedOption?.value })}
                    ...
                    <div className='mt-4'>
                        ...
                        <Select
                            options={supplierOptions}
                            value={supplierOptions.find((option) => option.value === currentPurchase.supplierId)}
                            onChange={(selectedOption) => setCurrentPurchase({ ...currentPurchase, supplierId: selectedOption?.value })}
                            placeholder="Select Supplier"
                            isClearable
                            className="text-black"
                        />
                    ...
                    <input type='date' value={currentPurchase?.transaction_date} onChange={(e) => setCurrentPurchase({ ...currentPurchase, transaction_date: e.target.value })} className='border border-gray-300 rounded p-1 text-black bg-white' />
                </div>
                        <div className='mt-4'>
                            <label className='block text-sm text-white'>Quantity:</label>
                            <input type='number' value={currentPurchase?.quantity} onChange={(e) => setCurrentPurchase({ ...currentPurchase, quantity: e.target.value })} className='border border-gray-300 rounded p-2 w-full mt-1 text-black' />
                        </div>
                        <div className='mt-4'>
                            <label className='block text-sm text-white'>Rate:</label>
                            <input type='number' value={currentPurchase?.rate} onChange={(e) => setCurrentPurchase({ ...currentPurchase, rate: e.target.value })} className='border border-gray-300 rounded p-2 w-full mt-1 text-black' />
                        </div>
                        <div className='mt-4'>
                            <label className='block text-sm text-white'>Paid Amount:</label>
                            <input type='number' value={currentPurchase?.paid_amount} onChange={(e) => setCurrentPurchase({ ...currentPurchase, paid_amount: e.target.value })} className='border border-gray-300 rounded p-2 w-full mt-1 text-black' />
                        </div>
                        {/* <div className='mt-4'>
            ...
            {isModalOpen1 && (
                <div className='fixed inset-0 top-72  flex items-center justify-center bg-black bg-opacity-50'>
                    <motion.div className='bg-gray-800 rounded-lg p-6 shadow-lg w-full md:w-1/3 lg:w-1/3' initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                        ...
                        <Select
                            options={productOptions}
                            value={productOptions.find((option) => option.value === createPurchase.productId)}
                            onChange={(selectedOption) => setCreatePurchase({ ...createPurchase, productId: selectedOption?.value })}
                            placeholder="Select Product"
                            isClearable
                            className="text-black"
                        />
                    ...
                    <div className='mt-4'>
                        ...
                        <Select
                            options={supplierOptions}
                            value={supplierOptions.find((option) => option.value === createPurchase.supplierId)}
                            onChange={(selectedOption) => setCreatePurchase({ ...createPurchase, supplierId: selectedOption?.value })}
                            placeholder="Select Supplier"
                            isClearable
                            className="text-black"
                        />
                        ...
                        <div className='mt-4'>
                    <label className='block text-sm text-white'> Date:</label>
                    <input type='date' value={createPurchase?.transaction_date} onChange={(e) => setCreatePurchase({ ...createPurchase, transaction_date: e.target.value })} className='border border-gray-300 rounded p-1 text-black bg-white' />
                </div>

                        <div className='mt-4'>
                            <label className='block text-sm text-white'>Quantity:</label>
                            <input type='number' value={createPurchase?.quantity} onChange={(e) => setCreatePurchase({ ...createPurchase, quantity: e.target.value })} className='border border-gray-300 rounded p-2 w-full mt-1 text-black' />
                        </div>
                        <div className='mt-4'>
                            <label className='block text-sm text-white'>Rate:</label>
                            <input type='number' value={createPurchase?.rate} onChange={(e) => setCreatePurchase({ ...createPurchase, rate: e.target.value })} className='border border-gray-300 rounded p-2 w-full mt-1 text-black' />
                        </div>
                        <div className='mt-4'>
                            <label className='block text-sm text-white'>Paid Amount:</label>
                            <input type='number' value={createPurchase?.paid_amount} onChange={(e) => setCreatePurchase({ ...createPurchase, paid_amount: e.target.value })} className='border border-gray-300 rounded p-2 w-full mt-1 text-black' />
                        </div>
                        {/* <div className='mt-4'>
                            <label className='block text-sm text-white'>Due Amount:</label>
                        ...
                        <div className='mt-6 flex justify-end'>
                            <button className='bg-indigo-600 hover:bg-indigo-700 text-white p-2 rounded mr-2' onClick={handlecreatePurchase}>Save</button>
